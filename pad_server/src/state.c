...
void padstate_init(padstate_t *state) {
    ...
    sem_init(&state->read_try, 0, MAX_READERS);
    sem_init(&state->resource, 0, 0);
...
*  
*/
int padstate_get_arm(padstate_t *state, arm_lvl_e *arm_val){ // is using a pointer to a variable good practice?
    int err;
